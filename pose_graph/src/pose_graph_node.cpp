...
queue<nav_msgs::Odometry::ConstPtr> velocity_buf;//add
queue<nav_msgs::Odometry::ConstPtr> acc_buf;//add
queue<nav_msgs::Odometry::ConstPtr> gyc_buf;//add
...
void velocity_callback(const nav_msgs::Odometry::ConstPtr &velocity_msg)
{
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
    velocity_buf.push(velocity_msg);
    /*printf("velo is:\n%f,%f,%f",velocity_msg->pose.pose.position.x,
                                velocity_msg->pose.pose.position.y,
                                velocity_msg->pose.pose.position.z);*/
                            
    m_buf.unlock();
}

void acc_callback(const nav_msgs::Odometry::ConstPtr &acc_msg)
{
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
...
void gyc_callback(const nav_msgs::Odometry::ConstPtr &gyc_msg)
{
    if(!LOOP_CLOSURE)
        return;
    m_buf.lock();
    gyc_buf.push(gyc_msg);
    /*printf("velo is:\n%f,%f,%f",velocity_msg->pose.pose.position.x,
                                velocity_msg->pose.pose.position.y,
                                velocity_msg->pose.pose.position.z);*/
    m_buf.unlock();
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    //printf("%f p is:%f,%f.%f,%f\n\n",pose_msg->header.stamp.toSec(),vio_q.x(),vio_q.y(),vio_q.z(),vio_q.w());
    vio_t = posegraph.w_r_vio * vio_t + posegraph.w_t_vio;
    vio_q = posegraph.w_r_vio *  vio_q;
    ...
    //if(pose_msg->child_frame_id == "Spline")
...
void process()
{
    ...
    while (true)
    {
        ...
        nav_msgs::Odometry::ConstPtr velocity_msg = NULL;
        nav_msgs::Odometry::ConstPtr acc_msg = NULL;
        nav_msgs::Odometry::ConstPtr gyc_msg = NULL;

        // find out the messages with same time stamp
        m_buf.lock();
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty() && !velocity_buf.empty() && !acc_buf.empty() && !gyc_buf.empty())
        {
           // printf("time is:%f\n\n",pose_buf.front()->header.stamp.toSec());
                ...
                && point_buf.back()->header.stamp.toSec() >= pose_buf.front()->header.stamp.toSec())
            {
                ...
                velocity_msg = velocity_buf.front();
                acc_msg = acc_buf.front();
                gyc_msg = gyc_buf.front();
                pose_buf.pop();
                velocity_buf.pop();
                acc_buf.pop();
                gyc_buf.pop();
                while (!pose_buf.empty()||!velocity_buf.empty()||!acc_buf.empty()||!gyc_buf.empty()){
                    pose_buf.pop();
                    velocity_buf.pop();
                    acc_buf.pop();
                    gyc_buf.pop();
        ...
        if (pose_msg != NULL)
        {
            ...
            Matrix3d R = Quaterniond(pose_msg->pose.pose.orientation.w,
                                     ...
                                     pose_msg->pose.pose.orientation.y,
                                     pose_msg->pose.pose.orientation.z).toRotationMatrix();
            Vector3d V = Vector3d(velocity_msg->pose.pose.position.x,
                                  velocity_msg->pose.pose.position.y,
                                  velocity_msg->pose.pose.position.z);
            Vector3d ACC = Vector3d(acc_msg->pose.pose.position.x,
                                  acc_msg->pose.pose.position.y,
                                  acc_msg->pose.pose.position.z);
            Vector3d GYC = Vector3d(gyc_msg->pose.pose.position.x,
                                  gyc_msg->pose.pose.position.y,
                                  gyc_msg->pose.pose.position.z);                      


            /*if(pose_msg->header.stamp.toSec()!=acc_msg->header.stamp.toSec()){
                printf("pose msg time is :%f\n",pose_msg->header.stamp.toSec());
                printf("velocity msg time is :%f\n\n",velocity_msg->header.stamp.toSec());
            }*/

            if((T - last_t).norm() > SKIP_DIS)//当位移大于一定值时才会视为keyframe
            {
                ...
                std::string frame_id_kf;
                std::string frame_id_kf_op;


                frame_id_kf=velocity_msg->header.frame_id;

                if(spline_lable){
                    char str[10] = "PoseGraph";
                    frame_id_kf = str;
                    segment_i = 0;
                    spline_lable = false;
                }
                ...
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, image,
                                   point_3d, point_2d_uv, point_2d_normal, point_id, sequence,frame_id_kf.c_str(),frame_id_kf_op.c_str(),V,ACC,GYC);   
                m_process.lock();
                start_flag = 1;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_veocity = n.subscribe("/vins_estimator/keyframe_velocity", 2000, velocity_callback);
    ros::Subscriber sub_acc = n.subscribe("/vins_estimator/keyframe_acc", 2000, acc_callback);
    ros::Subscriber sub_gyc = n.subscribe("/vins_estimator/keyframe_gyc", 2000, gyc_callback);
    //ros::Subscriber sub_gyc = n.subscribe("/vins_estimator/keyframe_velocity", 2000, gyc_callback);
